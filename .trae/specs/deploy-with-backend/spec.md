...
## Why
泽怀影像工作室官网目前仅能在本地开发环境访问，无法被搜索引擎收录，也缺少后端数据管理能力。需要将站点部署到公网使其可被搜索到，并接入后端数据库以支撑客户预约、内容管理等动态业务需求。

## What Changes
- 使用腾讯云开发（CloudBase）托管静态站点，实现公网可访问
- 配置 SEO 优化（meta 标签、Open Graph、sitemap、robots.txt），使搜索引擎可收录
- 接入 CloudBase 云数据库（NoSQL），存储业务数据
- 开发云函数 API 层，连接前端与数据库
- 前端新增预约提交等交互功能，对接后端 API

## Impact
- Affected specs: build-zehuai-website（在其基础上扩展后端能力）
- Affected code: 新增后端 API 层、修改前端部分组件以对接 API、新增 SEO 配置
...
### Requirement: 公网部署
系统 SHALL 将构建产物部署至腾讯云开发（CloudBase）静态网站托管，使网站可通过公网 URL 访问。
...
#### Scenario: 数据写入
- **WHEN** 前端提交预约或咨询表单
- **THEN** 数据通过云函数 API 写入对应集合，返回成功状态

### Requirement: 云函数 API
系统 SHALL 提供以下云函数 API 接口：
- `POST /api/bookings`：提交客户预约
- `POST /api/contacts`：提交客户咨询
- `GET /api/activities`：获取活动列表

#### Scenario: 提交预约
- **WHEN** 用户填写预约表单并提交
- **THEN** 调用云函数 API，数据写入 bookings 集合，返回预约编号

#### Scenario: 获取活动列表
- **WHEN** 前端请求活动数据
- **THEN** 返回 activities 集合中的有效活动列表
...
- **THEN** 表单数据通过 API 存入数据库，页面显示提交成功提示
